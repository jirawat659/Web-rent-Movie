...
// ----- บันทึกข้อมูลการเช่าในฐานข้อมูล และลด stock ----- 
foreach ($_SESSION['cart'] as $movie_ID => $movie) {
    $movie_id = $movie_ID;
    $date_start = $current_date;
    $date_return = $return_date;
    $rent_status = "ยังไม่คืน";

    // 1. บันทึกข้อมูลการเช่า
    ...
    // 2. ลด stock ทีละ 1 (เฉพาะถ้ายังมี)
    $updateStockStmt = $conn->prepare("UPDATE movie SET movie_stock = movie_stock - 1 WHERE movie_ID = ? AND movie_stock > 0");
    $updateStockStmt->bind_param("i", $movie_id);
    $updateStockStmt->execute();

    // 3. ตรวจสอบ stock เหลือ 0 ไหม ถ้าใช่ให้ปิดสถานะ
    $checkStockStmt = $conn->prepare("SELECT movie_stock FROM movie WHERE movie_ID = ?");
    $checkStockStmt->bind_param("i", $movie_id);
    $checkStockStmt->execute();
    $checkStockResult = $checkStockStmt->get_result();

    if ($row = $checkStockResult->fetch_assoc()) {
        if ($row['movie_stock'] == 0) {
            $updateStatusStmt = $conn->prepare("UPDATE movie SET movie_status = 0 WHERE movie_ID = ?");
            $updateStatusStmt->bind_param("i", $movie_id);
